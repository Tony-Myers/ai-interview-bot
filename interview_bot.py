...
if authentication_status:
    ...
    def generate_response(prompt, conversation_history=None):
        try:
            ...
            system_content = (
                "You are an experienced and considerate interviewer in higher education, focusing on AI applications. "
                "Use British English in your responses, including spellings like 'democratised'. Ensure your responses are complete and not truncated. "
                "After each user response, provide brief feedback and ask a relevant follow-up question based on their answer. "
                "Tailor your questions to the user's previous responses, avoiding repetition and exploring areas they haven't covered. "
                "Be adaptive and create a natural flow of conversation."
            )
    ...
    def main():
        ...
        if "current_question" not in st.session_state:
            st.session_state.current_question = (
                "Let's begin the interview. Can you please introduce yourself, your role in higher education, and your interest in AI?"
            )
        ...
        consent = st.checkbox(
            "I have read the information sheet and give my consent to participate in this interview."
        )
        ...
        if consent:
            ...
            user_answer = st.text_area(
                "Your response:", key=f"user_input_{len(st.session_state.conversation)}"
            )

            # Progress bar with a label indicating interview progress
            completed_questions = len(
                [entry for entry in st.session_state.conversation if entry['role'] == "user"]
            )
            progress_percentage = completed_questions / total_questions
            st.write(
                f"**Interview Progress: {completed_questions} out of {total_questions} questions answered**"
            )
            ...
            if st.button("Submit Answer"):
                if user_answer:
                    ...
                    ai_response = generate_response(
                        ai_prompt, st.session_state.conversation
                    )
            ...
            if st.checkbox("Show Interview Transcript"):
                ...
                st.markdown(
                    get_transcript_download_link(st.session_state.conversation),
